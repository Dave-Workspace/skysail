...
file_put_contents($logFile, "[".date('Y-m-d H:i:s')."] Super Visa Raw INPUT: $rawInput".PHP_EOL, FILE_APPEND | LOCK_EX);
...
try {
    ...
    if (mail($companyEmail, $subjectCompany, $messageCompany, $headers)) {
        file_put_contents($logFile, "[".date('Y-m-d H:i:s')."] COMPANY EMAIL SENT".PHP_EOL, FILE_APPEND | LOCK_EX);
    } else {
        file_put_contents($logFile, "[".date('Y-m-d H:i:s')."] COMPANY EMAIL FAILED".PHP_EOL, FILE_APPEND | LOCK_EX);
    }
